    ...
    <Modal show={show} onHide={onHide} centered size="lg">
      ...
      <Modal.Body>
        <Form onSubmit={handleSubmit}>
          <Row className="g-3">
            <Col md={12}><Form.Group><Form.Label className="small fw-bold text-muted">CONTRACT/PO ID *</Form.Label><Form.Control type="text" value={form.contractId} onChange={e => setForm(f => ({ ...f, contractId: e.target.value }))} required className="rounded-3" placeholder="PO-XXXXX" /></Form.Group></Col>
          ...
          <div className="d-flex justify-content-end gap-2 mt-4">
            ...
            <Button variant="primary" type="submit" className="rounded-3 px-4" disabled={submitting}>
                ...
                {invoices.map(invoice => {
                  ...
                  return (
                    <tr key={invoice.id}>
                      ...
                      <td className="small">{new Date(invoice.dueDate).toLocaleDateString()}</td>
      ...
      <Modal show={showEdit} onHide={() => setShowEdit(false)} centered size="lg">
        ...
        <Modal.Body>
          <Form onSubmit={(e) => { e.preventDefault(); handleEdit(); }}>
            <Row className="g-3">
              <Col md={12}><Form.Group><Form.Label className="small fw-bold text-muted">INVOICE NUMBER *</Form.Label><Form.Control type="text" value={editForm.invoiceNumber} onChange={e => setEditForm(f => ({ ...f, invoiceNumber: e.target.value }))} required className="rounded-3" /></Form.Group></Col>
        ...
        <Modal.Body>
          ...
          {selectedInvoice && (
            <div className="bg-light p-3 rounded-3">
              ...
              <div className="mb-1"><strong>Status:</strong> <Badge bg={INVOICE_STATUS_CONFIG[selectedInvoice.status]?.bg}>{INVOICE_STATUS_CONFIG[selectedInvoice.status]?.label}</Badge></div>
      ...
      <Modal show={showDetail} onHide={() => setShowDetail(false)} centered size="lg">
        ...
        {selectedInvoice && (
          <Modal.Body className="p-4">
            ...
            <div className="mb-3">
              <div className="small text-muted fw-bold mb-1">Line Items</div>
